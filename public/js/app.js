  ...
  images.forEach(image => {
    imagesHTML += `
      <div class="uploaded-image-item">
        <img src="${image.url || image.path}" alt="${image.prompter}'s image" class="uploaded-image">
        <div class="uploaded-image-info">
          <div class="uploaded-prompter">Prompter: ${image.prompter}</div>
          <div class="uploaded-votes">Votes: ${image.votes}</div>
        </div>
      </div>
    `;
  });
...
if (uploadForm) {
  uploadForm.addEventListener('submit', function(e) {
    ...
    fetch('/api/images')
      ...
      .then(images => {
        if (images.length >= 2) {
          showToast('Maximum of 2 images already uploaded. Please delete an image before uploading a new one.');
          return;
        }
  ...
  images.forEach(image => {
    voteHTML += `
      <div class="vote-card" data-id="${image.id}">
        ...
        <button class="btn btn-vote vote-button" data-id="${image.id}">Vote</button>
      </div>
    `;
  });
...
function renderAdminContent(images) {
  ...
  if (images.length === 0) {
    ...
    const resetPrompter1Btn = adminContainer.querySelector('.reset-votes-prompter1-btn');
    const resetPrompter2Btn = adminContainer.querySelector('.reset-votes-prompter2-btn');
  ...
  images.forEach(image => {
    adminHTML += `
      <div class="admin-card">
        <img src="${image.url || image.path}" alt="${image.prompter}'s image" class="admin-image">
...
async function resetPrompterVotes(prompterNumber) {
  if (confirm(`Are you sure you want to reset votes for Prompter ${prompterNumber}? This cannot be undone.`)) {
    try {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  filePreviewAreas.forEach(area => {
    const fileInput = area.closest('.file-input-container').querySelector('input[type="file"]');
    if (fileInput) {
